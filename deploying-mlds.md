...
## [ ](https://github.com/IHTSDO/MLDS/blob/master/src/main/documentation/Build-deploy.md#build-dependencies)Build dependencies:
...
Note: If JHipster tell you it requires at least JDK 1.7 and you think you have that already, then check what's actually being used by typing mvn -version If maven is using the wrong version of java, then you might want to set the JAVA_HOME environmental variable in your ~/.bash_profile file.

The war file is modified during the build by the spring-boot-maven-plugin to embed the Tomcat web server. This turns the war into an self-contained executable jar file (see [pom.xml](https://github.com/IHTSDO/MLDS/blob/master/pom.xml) )
...
Once the application is packaged as an executable war file, we then generate a Debian installable .deb file using the jdeb plugin. The contents and config for this are in the pom.xml xml and in the [src/main/deb](https://github.com/IHTSDO/MLDS/blob/master/src/main/deb) folder.
...
The application provides default configuration values in [config.properties](https://github.com/IHTSDO/MLDS/blob/master/src/main/deb/config.properties). Comments inline.
